...
QEMU_BINARY=/root/Drywall/build/qemu-system-x86_64
...
exec $QEMU_BINARY \
    ...
    -smp 4 \
    -M q35,cxl=on \
    -kernel /root/CXLMemSim/build/bzImage \
    -append "root=/dev/sda rw console=ttyS0,115200 nokaslr" \
    -drive file=/root/CXLMemSim/build/qemu.img,index=0,media=disk,format=raw \
